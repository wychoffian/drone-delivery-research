...
Phase E1 assigns physical coordinates to the synthetic network and integrates moving-source exposure across every route segment. Phase E2 connects each nine-receiver energy vector to the fleet model's reservation and completion events while preserving the current accounting rules. Phase E3 completes the first matched-seed sensitivity experiment covering acoustic uncertainty and declared budget scenarios. Phase E4 diagnoses the P3-D reference result as a stable budget trajectory crossing a discrete route-feasibility threshold near target 14. Phase E5 shows that slow damping loses its early advantage and that the greedy dispatcher produces non-monotonic fixed-budget service. Phase E6 quantifies the allocation gap against an integer optimization upper bound. Phase E7 executes request-level optimized routes in the event model. Phase E8 adds route-specific mission and recharge workload. Phase E9 implements online rolling dispatch with arrived requests and carried queues. Phase E10 reconnects Rolling 3 to adaptive control and identifies target saturation and structural overload. The next milestone is a demand and fleet capacity envelope for a stable pilot configuration.
