...
The design varies factors one group at a time in two concentrated-population scenarios. It does not cover all factor interactions, alternative network geometries, weather, failures, delivery deadlines, or measured acoustics. The Phase D progression gate remains open because conclusions depend materially on fleet capacity and exposure scaling. The next sensitivity extension should test destination probabilities and joint interactions among the factors that changed the results. Empirical readiness still requires a physical exposure definition, source data, and validation targets.
